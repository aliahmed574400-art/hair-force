...
const REQUIRED_ENV_VARS = {
  // Database — required in production only; dev falls back to in-memory demo store.
  DATABASE_URL: {
  ...
  // Session — required in production only; dev auto-generates and persists to
  // .dev-session-secret so logins survive restarts.
  SESSION_SECRET: {
    requiredInProduction: true,
...
function validateEnvVar(name, config = {}) {
  ...
  const treatAsRequired = config.required || (config.requiredInProduction && isProduction);
