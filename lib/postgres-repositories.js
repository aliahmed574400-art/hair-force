...
export const SESSION_MAX_AGE_SECONDS = AUTH_SESSION_MAX_AGE_SECONDS;
...
async function syncVendorMapLocation(nextVendor, existingVendor) {
  ...
  try {
    ...
    throw new Error(
      "We couldn't place this location on the map. Enter a clearer store location, city, or state and save again."
    );
  }
}
...
function mapVendorRow(row) {
  ...
  return hydrateVendorWindows({
    ...
    portfolioImages: parseJsonField(row.portfolio_images, parseJsonField(row.gallery_images, [])),
    specialties: parseJsonField(row.specialties, []),
    amenities: parseJsonField(row.amenities, []),
    serviceLocationType: row.service_location_type || "studio",
...
function mapServiceRow(row) {
  ...
  return {
    ...
    bookingMethod: row.booking_method || "instant",
...
function normalizeServicePayload(payload) {
  return {
    ...
    bookingMethod: payload.bookingMethod === "approval" ? "approval" : "instant",
...
function normalizeVendorPayload(existingVendor, payload) {
  ...
  const nextLocation = normalizeVendorLocationFields({
    state: payload.state ?? existingVendor.state,
    city: payload.city ?? existingVendor.city,
    area: payload.area ?? existingVendor.area,
    location: payload.location ?? existingVendor.location
  });
  ...
  return {
    ...
    portfolioImages: Array.isArray(payload.portfolioImages)
    ...
    serviceLocationType: String(
      payload.serviceLocationType || existingVendor.serviceLocationType || "studio"
    ).trim(),
...
function buildClientProfile(user) {
  return {
    ...
    canChangePassword: Boolean(user.email)
  };
}
...
async function createOrGetBookingConversation(booking) {
  ...
  const { rows } = await queryPostgres(
    `
      ...
      SET client_id = COALESCE(booking_conversations.client_id, EXCLUDED.client_id),
          updated_at = NOW()
      RETURNING *
    `,
...
async function listBookingConversationsForUser(user) {
  ...
  if (!hasDatabase) {
    ...
    const conversations = (store.conversations || [])
      .map((conversation) => {
        const booking = (store.bookings || []).find((item) => item.id === conversation.bookingId);
        const messages = (store.messages || []).filter((item) => item.conversationId === conversation.id);
        return booking ? decorateConversationRecord(conversation, booking, messages) : null;
      })
      .filter(Boolean)
...
export async function getBookingConversationForUser(user, conversationId) {
  ...
  if (!hasDatabase) {
    ...
    const booking = (store.bookings || []).find((item) => item.id === rawConversation.bookingId);
  ...
  await markConversationReadForUser(user, conversation);
  const nextRow = await getConversationRowById(conversationId);
  const messages = (await getMessageRowsByConversationId(conversationId)).map(mapBookingMessageRow);

  return {
    conversation: mapConversationRow(nextRow || row),
    messages
  };
}
...
export async function sendBookingMessage(user, conversationId, payload) {
  ...
  if (user.role === "vendor" && conversation.clientId) {
    await createClientNotification({
      ...
      title: "New message from your stylist",
      message: `${conversation.vendorName} sent an update about your ${conversation.serviceName} booking.`,
...
async function getServiceRowsByVendorSlugs(vendorSlugs, orderBy = "featured DESC, price ASC") {
  ...
  const result = await queryPostgres(
    `SELECT * FROM services WHERE vendor_slug = ANY($1::text[]) ORDER BY ${orderBy}`,
    [vendorSlugs]
  );
  return result.rows;
}
...
async function getConversationRowById(conversationId) {
  const result = await queryPostgres(
    `
      ...
      INNER JOIN bookings ON bookings.id = booking_conversations.booking_id
...
export async function searchDiscoverStylists(filters = {}) {
  ...
  if (!hasDatabase) {
    ...
    const activeServices = store.services.filter((service) => service.isActive !== false);
    ...
    for (const service of serviceRows.map(mapServiceRow).filter((item) => item.isActive !== false)) {
      const current = servicesByVendor.get(service.vendorSlug) || [];
      current.push(service);
      servicesByVendor.set(service.vendorSlug, current);
    }
...
export async function signupUser(payload) {
  ...
  try {
    ...
    const { rows } = await queryPostgres(
      `
        INSERT INTO users (
          id, name, email, phone, phone_normalized, city, vendor_slug, role, password_hash, avatar, created_at, updated_at
  ...
  return withPostgresTransaction(async (db) => {
    ...
    const userResult = await db.query(
      `
        INSERT INTO users (
          id, name, email, phone, phone_normalized, city, vendor_slug, role, password_hash, avatar, created_at, updated_at
        ) VALUES ($1, $2, $3, $4, $5, $6, $7, 'vendor', $8, $9, NOW(), NOW())
        RETURNING *
      `,
      [
        userId,
    ...
    const vendorResult = await db.query(
      `
        INSERT INTO vendor_profiles (
          ...
          $16::jsonb, $17::jsonb, '[]'::jsonb, 'studio', $18::jsonb, '{}'::jsonb, 'pending', NOW(), NOW()
        )
        RETURNING *
      `,
      [
...
export async function createBooking(payload) {
  ...
  if (!hasDatabase) {
    ...
    await createOrGetBookingConversation(bookingRecord);
  ...
  await createOrGetBookingConversation(booking);
...
export async function getDashboardDataForUser(user) {
  ...
  if (user.role === "vendor") {
    ...
    if (!hasDatabase) {
      ...
      const conversations = await listBookingConversationsForUser(user);
...
export async function changeClientPassword(user, payload) {
  ...
  if (!user.email) {
    throw new Error("Password changes are only available for email-based accounts.");
  }
...
export async function addClientPaymentMethod(user, payload) {
  ...
  if (!hasDatabase) {
    ...
    await createClientNotification({
      ...
      message: `${method.brand} ending in ${method.last4} is ready for upcoming bookings.`,
...
export async function setDefaultClientPaymentMethod(user, methodId) {
  assertClientUser(user);
  const method = await getClientPaymentMethodById(user, methodId);
