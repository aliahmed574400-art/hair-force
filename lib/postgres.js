...
function getPool() {
  ...
  if (!globalThis.hairforcePostgresCache.pool) {
    ...
    globalThis.hairforcePostgresCache.pool = pool;
  }
  ...
  for (const service of services) {
    await client.query(
      ...
      [
        ...
        service.bookingMethod || "approval",
  ...
  for (const booking of bookings) {
    await client.query(
      ...
      [
        ...
        booking.bookingMethod || "approval",
...
export async function queryPostgres(text, params = []) {
  ...
  try {
    ...
    if (isRetriableConnectionError(error)) {
      console.warn("PostgreSQL connection lost, retrying once:", error.message);
      try {
        return await getPool().query(text, params);
      } catch (retryError) {
        console.error("PostgreSQL retry failed:", retryError.message);
        throw retryError;
      }
    }
...
async function runPostgresTransaction(callback, allowSchemaRetry = true, allowConnectionRetry = true) {
  const pool = await ensurePostgresReady();
  ...
  try {
    ...
    if (allowConnectionRetry && isRetriableConnectionError(error)) {
      console.warn("PostgreSQL connection lost on connect, retrying once:", error.message);
      return runPostgresTransaction(callback, allowSchemaRetry, false);
    }
    throw error;
  }
  ...
  try {
    ...
    if (allowSchemaRetry && isSchemaMismatchError(error)) {
      globalThis.hairforcePostgresCache.initPromise = null;
      await ensurePostgresReady();
      return runPostgresTransaction(callback, false, allowConnectionRetry);
    }
