...
export async function sendBookingMessage(user, conversationId, payload) {
  ...
  if (user.role !== "vendor") {
    await createVendorNotification({
      ...
      message: `${conversation.customerName || "A client"} sent a message about ${conversation.serviceName}.`,
      bookingId: conversation.bookingId,
      conversationId: conversation.id,
  ...
  await db.query(
    `
      INSERT INTO vendor_profiles (
        ...
        $17::jsonb, '[]'::jsonb, 'studio', $18::jsonb, '{}'::jsonb, 'pending', NOW(), NOW()
      )
    `,
    [
      vendorId,
    ...
    const vendorResult = await db.query(
      `
        INSERT INTO vendor_profiles (
          ...
          '', '', NULL, NULL, 'approx_area', 'pending', NOW(), NOW()
        )
        RETURNING *
      `,
      [
...
export async function getDashboardDataForUser(user) {
  ...
  if (user.role === "vendor") {
    ...
    const [serviceRows, bookingRows, conversations, notificationPreview] = await Promise.all([
