...
export async function sendPasswordChangeOtpEmail({ to, code, expiresInSeconds }) {
  const resendApiKey = String(process.env.RESEND_API_KEY || "").trim();
  const resendFromEmail = String(process.env.RESEND_FROM_EMAIL || "").trim();
  const expiresInMinutes = Math.max(1, Math.ceil(Number(expiresInSeconds || 0) / 60));
  ...
  if (!resendApiKey || !resendFromEmail) {
    if (process.env.NODE_ENV === "production") {
      throw new Error("Password change email is not configured yet.");
    }
  ...
  const response = await fetch(RESEND_API_URL, {
    ...
    body: JSON.stringify({
      from: resendFromEmail,
      to: [String(to || "").trim()],
      subject: "Your Hair Force password change code",
      text: `Your Hair Force password change verification code is ${code}. It expires in ${expiresInMinutes} minute${expiresInMinutes === 1 ? "" : "s"}.`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#0f172a;">
          <p style="margin:0 0 12px;font-size:14px;letter-spacing:.08em;text-transform:uppercase;color:#2563eb;">
            Hair Force
          </p>
          <h1 style="margin:0 0 12px;font-size:28px;line-height:1.2;">Change your password</h1>
          <p style="margin:0 0 18px;font-size:16px;line-height:1.6;">
            Use this verification code to confirm your password change request.
          </p>
          <div style="margin:0 0 18px;padding:18px 20px;border-radius:18px;background:#eff6ff;border:1px solid #bfdbfe;font-size:32px;font-weight:700;letter-spacing:.32em;text-align:center;">
            ${codeMarkup}
          </div>
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(errorText || "Password change email could not be sent.");
  }
