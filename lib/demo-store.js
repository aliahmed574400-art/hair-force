...
function normalizeServices(vendor) {
  return vendor.services.map((service, index) => ({
    ...
    metadata: {
      priceIsStartingAt: Boolean(service.metadata?.priceIsStartingAt),
...
function createInitialStore() {
  const vendors = clone(stylists).map((vendor) => ({
    ...
    portfolioItems:
      ...
      (vendor.portfolioImages || vendor.galleryImages || []).map((url, index) => ({
        id: `media-${vendor.slug}-${index + 1}`,
        url,
        type: "image",
        serviceId: "",
        clientName: "",
    ...
    personalInfo: vendor.personalInfo || {
      ...
      pronouns: [],
      profession: vendor.category || "Hair Stylist",
      about: vendor.bio || "",
      email: "",
      phone: "",
      websitePath: `/stylists/${vendor.slug}`
    },
    businessInfo: vendor.businessInfo || {
      businessName: vendor.name,
      salonNumber: "",
      personalPhoneNumber: "",
      numberShownOnProfile: "salonNumber",
      smsNotificationsPhoneNumber: "",
      mobileBusiness: vendor.serviceLocationType === "mobile",
  ...
  const hydratedVendors = vendors.map((vendor) => ({
    ...
    bookingWindows:
      vendor.bookingWindows?.length || !(vendor.availabilityRules || []).length
        ...
        : buildBookingWindowsFromRules(vendor.availabilityRules, vendor.blackoutDates || [], 21, 6, {
            availabilityOverrides: vendor.availabilityOverrides || []
          })
  }));

  if (!hydratedVendors.length) {
    return {
      vendors: [],
      services: [],
      users: [
  ...
  const bookings = [
    {
      ...
      bookingMethod: firstService.bookingMethod || "approval",
      status: "confirmed",
      source: "web",
    ...
    },
    {
      ...
      bookingMethod: thirdService.bookingMethod || "approval",
      status: "completed",
      source: "web",
  ...
  const vendorNotifications = [
    {
      ...
      message: `${bookings[1].customerName} requested ${bookings[1].serviceName} for ${bookings[1].appointmentDate} at ${bookings[1].appointmentSlot}.`,
      bookingId: bookings[1].id,
      conversationId: "",
      clientName: bookings[1].customerName,
      clientAvatar: "",
      serviceName: bookings[1].serviceName,
      appointmentDate: bookings[1].appointmentDate,
      appointmentSlot: bookings[1].appointmentSlot,
      metadata: {
        bookingId: bookings[1].id,
        bookingStatus: bookings[1].status
      },
      readAt: null,
      createdAt: shiftTimestamp(-1, 11),
      updatedAt: shiftTimestamp(-1, 11)
    },
    {
      id: "vntf-demo-102",
      vendorSlug: primaryVendor.slug,
      ...
      message: `${users[0].name} sent a message about ${bookings[0].serviceName}.`,
      bookingId: bookings[0].id,
      ...
      appointmentSlot: bookings[0].appointmentSlot,
      metadata: {
        bookingId: bookings[0].id,
        conversationId: conversations[0].id,
    ...
    },
    {
      ...
      message: `${users[0].name} booked ${bookings[0].serviceName} for ${bookings[0].appointmentDate} at ${bookings[0].appointmentSlot}.`,
      bookingId: bookings[0].id,
      conversationId: conversations[0].id,
      clientName: users[0].name,
      clientAvatar: "",
